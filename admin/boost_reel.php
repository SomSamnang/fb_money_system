...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['boost_reel'])) {
    $title = trim($_POST['title']);
    $link = trim($_POST['reel_link']);
    ...
    if (!empty($title) && !empty($link)) {
        ...
        if ($stmt) {
            $stmt->bind_param("ssiiisis", $title, $link, $target, $points, $duration, $expires_at, $daily_limit, $platform);
            if ($stmt->execute()) {
                $message = "Reel successfully added for boosting!";
                $toast_class = "bg-success";
                logAction($conn, $_SESSION['admin'], 'Boost Reel', "Added reel: $title");
            } else {
...
<title>Boost Reel - FB Money System</title>
    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light bg-light border-bottom mb-4">
            <div class="container-fluid">
                ...
                <span class="navbar-text ms-auto fw-bold text-primary">Boost Reel</span>
            ...
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    <div class="card border-0 shadow-lg overflow-hidden rounded-4">
                        <div class="card-header text-white p-4 text-center" style="background: linear-gradient(135deg, #e83e8c 0%, #d63384 100%);">
                            <h2 class="fw-bold mb-1"><i class="bi bi-camera-reels-fill me-2"></i>Boost New Reel</h2>
                            <p class="mb-0 opacity-75">Promote your Facebook Reels instantly!</p>
                        </div>
                        <div class="card-body p-5">
                        <form method="POST">
                            <div class="mb-3"><label class="form-label fw-bold">Reel Title</label><div class="input-group"><span class="input-group-text"><i class="bi bi-type-h1"></i></span><input type="text" class="form-control form-control-lg" name="title" placeholder="e.g. Funny Cat Reel" required></div></div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Platform</label>
                                <div class="input-group"><span class="input-group-text"><i class="bi bi-globe"></i></span><select class="form-select form-select-lg" name="platform">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold">Reel Link</label>
                                <div class="input-group">
                                    <span class="input-group-text"><i class="bi bi-link-45deg"></i></span>
                                    <input type="url" class="form-control form-control-lg" name="reel_link" id="reel_link" placeholder="https://www.facebook.com/reel/..." required>
                                    <button type="button" class="btn btn-outline-secondary" onclick="previewReel()"><i class="bi bi-eye"></i> Preview</button>
                                </div>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3"><label class="form-label fw-bold">Points Reward</label><div class="input-group"><span class="input-group-text"><i class="bi bi-coin"></i></span><input type="number" class="form-control" name="points_per_view" placeholder="10" value="10"></div></div>
                                <div class="col-md-6 mb-3"><label class="form-label fw-bold">Duration (Seconds)</label><div class="input-group"><span class="input-group-text"><i class="bi bi-stopwatch"></i></span><input type="number" class="form-control" name="duration" placeholder="30" value="30"></div></div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3"><label class="form-label fw-bold">Daily Limit (Optional)</label><div class="input-group"><span class="input-group-text"><i class="bi bi-speedometer2"></i></span><input type="number" class="form-control" name="daily_limit" placeholder="0 (No limit)" value="0"></div></div>
                                <div class="col-md-6 mb-3"><label class="form-label fw-bold">Campaign Duration (Days)</label><div class="input-group"><span class="input-group-text"><i class="bi bi-calendar-event"></i></span><input type="number" class="form-control" name="campaign_days" placeholder="1" value="1" min="1"></div></div>
                            </div>
                            <div class="d-grid"><button type="submit" name="boost_reel" class="btn btn-danger btn-lg fw-bold shadow-sm" style="background: linear-gradient(135deg, #e83e8c 0%, #d63384 100%); border:none;"><i class="bi bi-play-btn-fill me-2"></i>Start Boosting Reel</button></div>
                        </form>
                        </div>
                    </div>
                </div>
...
function previewReel() {
    const url = document.getElementById('reel_link').value;
    if (url) {
