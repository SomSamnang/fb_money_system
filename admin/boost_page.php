...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['boost_page'])) {
    ...
    if (!empty($name) && !empty($link)) {
        if (!filter_var($link, FILTER_VALIDATE_URL)) {
            ...
            if ($check->get_result()->num_rows > 0) {
                ...
                if ($stmt) {
                    $stmt->bind_param("ssi", $name, $link, $target);
                    if ($stmt->execute()) {
                        $message = "Page successfully added for boosting!";
                        $toast_class = "bg-success";
                        logAction($conn, $_SESSION['admin'], 'Boost Page', "Added page: $name");
                    } else {
...
<title>Boost Page - FB Money System</title>
    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light bg-light border-bottom mb-4">
            <div class="container-fluid">
                ...
                <span class="navbar-text ms-auto fw-bold text-primary">Boost Page</span>
