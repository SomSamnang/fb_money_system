...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['boost_view'])) {
    ...
    if (!empty($title) && !empty($link)) {
        ...
        if ($platform == 'youtube') {
            ...
            if ((strpos($link, 'facebook.com') !== false || strpos($link, 'fb.watch') !== false) && strpos($link, 'plugins/video.php') === false) {
                $link = "https://www.facebook.com/plugins/video.php?href=" . urlencode($link) . "&show_text=false&t=0";
            }
        }
        // 'other' platform keeps link as is

        $stmt = $conn->prepare("INSERT INTO videos (title, video_link, target_views, points_per_view, duration, expires_at, platform) VALUES (?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssiiiss", $title, $link, $target, $points, $duration, $expires_at, $platform);
            if ($stmt->execute()) {
                $message = "View campaign added successfully!";
                $toast_class = "bg-success";
                logAction($conn, $_SESSION['admin'], 'Boost View', "Added $platform view: $title");
            } else {
...
<title>Boost View - FB Money System</title>
    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light bg-light border-bottom mb-4">
            <div class="container-fluid">
                ...
                <span class="navbar-text ms-auto fw-bold text-primary">Boost View</span>
